...
class SlurmBot:
	...
	def run(self, cmd, dry=False, v=0, **kwargs):
		...
		params["name"] = params["name"] if ("name" in params.keys() and params["name"]) else cmd.split(" ")[0]

		# Build sbatch command
		sbatch_cmd = f'''sbatch {params["account"]} {params["partition"]} {params["reservation"]} {params["dependency"]} \
